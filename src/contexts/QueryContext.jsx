...
import Supervisor from '../utils/supervisor/Supervisor';
...
// Add this function after the isValidUUID function
const validateConversations = async (conversations, userId) => {
  ...
  for (const conv of conversations) {
    try {
      // Skip conversations without session IDs
      if (!conv.sessionId || !isValidUUID(conv.sessionId)) {
        console.warn(`Invalid session ID for conversation ${conv.id}: ${conv.sessionId}`);
        continue;
      }
      ...
      const { data: session, error } = await supabase
        ...
        .eq('id', conv.sessionId)
        .eq('user_id', userId)
        .single();

      if (error || !session) {
        console.warn(`Session ${conv.sessionId} not found in Supabase`);
        continue;
      }
      ...
      if (interactionsError) {
        console.warn(`Error checking interactions for session ${conv.sessionId}:`, interactionsError);
        continue;
      }

      // If we get here, the conversation is valid
      validConversations.push({
        ...conv,
        lastMessageTime: session.created_at // Update with actual session creation time
      });
    } catch (error) {
      console.error(`Error validating conversation ${conv.id}:`, error);
    }
  }
...
export function QueryProvider({ children }) {
  ...
  const supervisor = new Supervisor(0.3);
  ...
  const submitQuery = async (query, preferences = {}) => {
    try {
      ...
      // Analyze query using Supervisor
      const analysis = supervisor.analyzeQuery(query);
      console.log('QueryContext: Query analysis result:', analysis);
      ...
      try {
        ...
        console.log('QueryContext: Sending query to backend:', {
          query,
          sessionId,
          preferences,
          analysis,
        ...
        console.log('QueryContext: Raw response from backend:', JSON.stringify(response.data, null, 2));
        
        responseData = response.data;
        
        // Convert response object into array of messages
        const messages = [];
        
        // Add main response if available
        if (responseData.main) {
          console.log('QueryContext: Processing main response:', JSON.stringify(responseData.main, null, 2));
          const mainMessage = {
            role: 'assistant',
            type: 'main',
            id: responseData.main.id,
            sessionId: responseData.main.sessionId,
            query: responseData.main.query,
            suggested_title: responseData.main.suggested_title,
            introduction: responseData.main.introduction,
            explanation: responseData.main.explanation,
            analogy: responseData.main.analogy,
            resources: responseData.main.resources,
            recap: responseData.main.recap,
            timestamp: responseData.main.timestamp || new Date().toISOString()
          };
          console.log('QueryContext: Created main message:', JSON.stringify(mainMessage, null, 2));
          messages.push(mainMessage);
        }

        // Add suggestions if available
        if (responseData.suggestions) {
          console.log('QueryContext: Processing suggestions:', JSON.stringify(responseData.suggestions, null, 2));
          const suggestionsMessage = {
            role: 'assistant',
            type: 'suggestions',
            content: responseData.suggestions.content,
            timestamp: new Date().toISOString(),
            originalQuery: query
          };
          console.log('QueryContext: Created suggestions message:', JSON.stringify(suggestionsMessage, null, 2));
          messages.push(suggestionsMessage);
        }

        // Update message history with all messages
        console.log('QueryContext: Updating message history with messages:', JSON.stringify(messages, null, 2));
        setMessageHistory(prev => {
          const newHistory = [...prev, ...messages];
          console.log('QueryContext: New message history:', JSON.stringify(newHistory, null, 2));
          return newHistory;
        });
        
        // Set current response to main message for backward compatibility
        setCurrentResponse(responseData.main);
      } catch (err) {
  ...
  // Handle deepening request
  const handleDeepening = async (suggestionId, originalQuery, feedback = null) => {
    try {
      setLoading(true);
      setError(null);

      // Get the original analysis
      const analysis = supervisor.analyzeQuery(originalQuery);
      
      // Enhance the query based on the selected suggestion or feedback
      let enhancedQuery = originalQuery;
      if (feedback) {
        // Handle regeneration based on feedback
        let feedbackInstructions = [];
        
        if (feedback.explanationClear === 'no' || feedback.explanationClear === 'partially') {
          feedbackInstructions.push('Please provide a clearer and more straightforward explanation.');
        }
        
        if (feedback.explanationDetail === 'more_detailed') {
          feedbackInstructions.push('Please provide a more detailed and comprehensive explanation.');
        } else if (feedback.explanationDetail === 'simpler') {
          feedbackInstructions.push('Please provide a simpler and more concise explanation.');
        }
        
        if (feedback.analogyHelpful === 'no' || feedback.analogyHelpful === 'partially') {
          if (feedback.analogyPreference) {
            feedbackInstructions.push(`Please use a ${feedback.analogyPreference}-related analogy to explain the concept.`);
          } else {
            feedbackInstructions.push('Please provide a different analogy that might be more helpful.');
          }
        }
        
        if (feedback.comments) {
          feedbackInstructions.push(feedback.comments);
        }
        
        enhancedQuery = `${originalQuery} ${feedbackInstructions.join(' ')}`;
      } else {
        // Handle regular deepening suggestions
        switch (suggestionId) {
          case 'deeper':
            enhancedQuery = `${originalQuery} Please provide a more comprehensive explanation with advanced concepts and deeper insights.`;
            break;
          case 'examples':
            enhancedQuery = `${originalQuery} Please include detailed real-world examples and practical applications.`;
            break;
          case 'technical':
            enhancedQuery = `${originalQuery} Please provide a detailed technical breakdown with specific implementation details.`;
            break;
          case 'step-by-step':
            enhancedQuery = `${originalQuery} Please break this down into clear, sequential steps with detailed explanations for each step.`;
            break;
          default:
      ...
      const response = await axios.post(`${API_URL}/api/query`, {
        ...
        analysis: {
          ...analysis,
          needsDeepening: false, // Prevent recursive suggestions
          deepeningType: suggestionId,
          feedback: feedback // Include feedback in analysis if present
        }
      });

      // Add the enhanced response to message history
      if (response.data.main) {
        const enhancedMessage = {
          role: 'assistant',
          content: response.data.main.introduction + '\n\n' + response.data.main.explanation,
          analogy: response.data.main.analogy,
          resources: response.data.main.resources,
          recap: response.data.main.recap,
          timestamp: new Date().toISOString(),
          type: feedback ? 'regenerated' : 'enhanced',
          deepeningType: suggestionId,
          feedback: feedback // Store feedback with the message for reference
        };
        setMessageHistory(prev => [...prev, enhancedMessage]);
      }

      return response.data;
    } catch (err) {
      console.error('Error handling deepening:', err);
      setError('Failed to process deepening request. Please try again.');
      throw err;
    } finally {
      setLoading(false);
    }
  ...
  const loadConversationHistory = async (sessionId) => {
    try {
      ...
      if (!user) {
        throw new Error('You must be logged in to load conversation history');
      }
      ...
      // Transform the interactions into the expected message format
      const messages = sessionData.interactions.map(item => ({
        role: item.type === 'query' ? 'user' : 'assistant',
        content: item.type === 'query' ? item.query : item.response,
        timestamp: item.timestamp || item.created_at,
        type: item.type,
        ...(item.response && typeof item.response === 'object' ? item.response : {})
      }));
      ...
      if (user) {
        localStorage.setItem(`messageHistory_${user.id}`, JSON.stringify(messages));
      }
  ...
  // Update the loadConversations function to use validation
  const loadConversations = async () => {
    ...
    try {
      ...
      const validConversations = await validateConversations(savedConversations, user.id);
      
      // Update localStorage with validated conversations
      if (validConversations.length !== savedConversations.length) {
        console.warn('Some conversations were filtered out during validation');
        localStorage.setItem(`conversations_${user.id}`, JSON.stringify(validConversations));
      }
