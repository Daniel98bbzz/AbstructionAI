...
// Add a debug endpoint to check and fix user profiles
app.get('/api/debug/fix-user-profile/:userId', async (req, res) => {
  try {
    const { userId } = req.params;
    ...
    // Step 1: Check how many profiles exist for this user
    const { data, error } = await supabase
      .from('user_profiles')
    ...
    if (error) {
      ...
      return res.status(500).json({ 
        success: false, 
        message: 'Error checking profiles', 
        error: error.message 
      });
    }
    ...
    // Step 2: Take appropriate action based on results
    if (data.length === 0) {
      ...
      actionTaken = 'created';
      fixedProfile = createdData[0];
      
      // Also update the memory cache
      if (global.userProfiles) {
        global.userProfiles[userId] = fixedProfile;
      }
    } else if (data.length > 1) {
      console.log(`Found ${data.length} profiles for user ${userId}, fixing...`);
      
      // Keep the most recently updated profile
      data.sort((a, b) => new Date(b.updated_at) - new Date(a.updated_at));
      const profileToKeep = data[0];
      const profilesToDelete = data.slice(1);
      
      console.log(`Keeping profile from ${profileToKeep.updated_at}, deleting ${profilesToDelete.length} others`);
      ...
      for (const profile of profilesToDelete) {
        const { error: deleteError } = await supabase
          ...
          .delete()
          .eq('id', profile.id)
          .eq('created_at', profile.created_at); // Extra safety condition
      ...
      if (fixedError) {
        console.error('Error retrieving fixed profile:', fixedError);
      } else {
        fixedProfile = fixedData;
      ...
      actionTaken = 'deduplicated';
    } else {
      console.log(`Found exactly one profile for user ${userId}`);
      ...
      if (profileError) {
        ...
        const interests = ensureArray(profileData.interests);
        const domains = ensureArray(profileData.preferred_analogy_domains);
        
        // Update if necessary
        if (typeof profileData.interests === 'string' || typeof profileData.preferred_analogy_domains === 'string') {
          console.log('Fixing array format in profile');
          
          const { data: updatedData, error: updateError } = await supabase
            .from('user_profiles')
            .update({
              interests: interests,
              preferred_analogy_domains: domains
            })
            .eq('id', userId)
            .select();
          
          if (updateError) {
            console.error('Error updating profile arrays:', updateError);
          } else if (updatedData) {
            fixedProfile = updatedData[0];
    ...
    console.error('Error in fix-user-profile endpoint:', error);
    return res.status(500).json({
      success: false,
...
// Add an admin route to fix RLS policies
app.post('/api/admin/fix-rls-policies', async (req, res) => {
  try {
    const { adminKey } = req.body;
    // Simple security check - in production, use proper authentication
    if (adminKey !== 'fix-rls-admin') {
      return res.status(401).json({ 
        success: false, 
        message: 'Unauthorized - incorrect admin key' 
      });
    ...
    // Helper function to execute SQL that tries both function names
    async function executeSql(sql) {
      try {
        // Try execute_sql first
        const result = await supabase.rpc('execute_sql', { sql });
        if (result.error) {
          console.log('execute_sql returned error, trying exec_sql');
          return await supabase.rpc('exec_sql', { sql });
        }
        return result;
      } catch (error) {
        console.log('execute_sql failed, trying exec_sql');
        return await supabase.rpc('exec_sql', { sql });
      }
    }
    
    // Drop existing restrictive policies for interactions table
    console.log('Fixing interactions table policies...');
    
    const dropInteractionsPolicies = [
    ...
    for (const sql of dropInteractionsPolicies) {
    ...
    const { error: createError } = await executeSql(createInteractionsPolicy);
    if (createError) {
      console.error(`Error creating interactions policy: ${createError.message}`);
    }
    
    // Fix prompt_template_usage table
    console.log('Fixing prompt_template_usage table policies...');
    
    const dropTemplatePolicy = 'DROP POLICY IF EXISTS "Restrictive template usage policy" ON public.prompt_template_usage';
    const { error: dropTemplateError } = await executeSql(dropTemplatePolicy);
    
    if (dropTemplateError) {
      console.error(`Error dropping template policy: ${dropTemplateError.message}`);
    }
    ...
    const { error: createTemplateError } = await executeSql(createTemplatePolicy);
    if (createTemplateError) {
      console.error(`Error creating template policy: ${createTemplateError.message}`);
    }
    ...
    for (const sql of enableQueries) {
      const { error } = await executeSql(sql);
      if (error) {
    ...
    `;
    const { data: policies, error: verifyError } = await executeSql(verifyQuery);
    
    res.json({
      success: true,
      message: 'RLS policies have been fixed',
      policies: verifyError ? null : policies
    });
  } catch (error) {
...
// Add RLS admin UI
app.get('/rls-admin', (req, res) => {
  res.sendFile(path.join(__dirname, 'rlsFixAdmin.html'));
});

// Schedule periodic tasks for multi-signal metrics
// For now, just run them once on startup
console.log('[Crowd Wisdom] Setting up multi-signal metrics calculation');

// Schedule the first run after server startup
setTimeout(async () => {
  try {
    console.log('[Crowd Wisdom] Running initial multi-signal metrics calculation');
    
    // Calculate confidence scores
    const confidenceUpdated = await ResponseClusterManager.calculateConfidenceScores();
    console.log(`[Crowd Wisdom] Updated confidence scores for ${confidenceUpdated} templates`);
    
    // Calculate follow-up rates
    const followUpUpdated = await ResponseClusterManager.calculateFollowUpRates();
    console.log(`[Crowd Wisdom] Updated follow-up rates for ${followUpUpdated} templates`);
    
    // Calculate composite quality scores
    const compositeUpdated = await ResponseClusterManager.calculateCompositeQualityScores();
    console.log(`[Crowd Wisdom] Updated composite quality scores for ${compositeUpdated} templates`);
    
    // Schedule periodic updates every 6 hours
    setInterval(async () => {
      console.log('[Crowd Wisdom] Running scheduled multi-signal metrics update');
      await ResponseClusterManager.calculateConfidenceScores();
      await ResponseClusterManager.calculateFollowUpRates();
      await ResponseClusterManager.calculateCompositeQualityScores();
    }, 6 * 60 * 60 * 1000); // 6 hours
    
  } catch (error) {
    console.error('[Crowd Wisdom] Error in scheduled metrics calculation:', error);
  }
}, 30000); // Wait 30 seconds after server startup

// Add an admin route to manually recalculate composite quality scores
app.post('/api/admin/crowd-wisdom/recalculate-scores', async (req, res) => {
  try {
    const { adminKey, weights } = req.body;
    
    // Simple security check - in production, use proper authentication
    if (adminKey !== 'crowd-wisdom-admin') {
      return res.status(401).json({ 
        success: false, 
        message: 'Unauthorized - incorrect admin key' 
      });
    ...
    console.log('[Crowd Wisdom] Admin-triggered score recalculation with weights:', weights || 'default');
    
    // Get an instance of Supervisor to run the calculation
    const supervisor = new Supervisor();
    const updatedCount = await supervisor.updateCompositeScores(weights);
    
    res.json({
      success: true,
      message: `Recalculated composite scores for ${updatedCount} templates`,
      custom_weights: !!weights
    });
  } catch (error) {
    console.error('Error recalculating composite scores:', error);
    res.status(500).json({ 
      success: false, 
      error: error.message 
    });
...
// Add A/B testing toggle for use of composite score vs. efficacy score
app.post('/api/query', async (req, res) => {
  try {
    const { query, sessionId, preferences, feedback, isRegeneration, originalResponseId, abTestGroup } = req.body;
...
// Add an endpoint to get A/B testing statistics
app.get('/api/admin/crowd-wisdom/ab-stats', async (req, res) => {
  try {
    // Get usage statistics by selection method
    const { data: usageData, error: usageError } = await supabase
      .from('prompt_template_usage')
      .select('id, template_id, feedback_score, response_id, created_at');
    
    if (usageError) throw usageError;
    
    // Get all responses with crowd wisdom data
    const { data: interactions, error: interactionsError } = await supabase
      .from('interactions')
      .select('message_id, feedback_content')
      .filter('feedback_content', 'not.is', null)
      .limit(1000);
      
    if (interactionsError) throw interactionsError;
    
    // Fetch templates to get selection methods
    const { data: templates, error: templatesError } = await supabase
      .from('prompt_templates')
      .select('id, efficacy_score, composite_quality_score');
      
    if (templatesError) throw templatesError;
    
    // Match response IDs to templates and compile stats
    const stats = {
      efficacy_score: {
        count: 0,
        ratings: [],
    ...
    // Process usage data to compile statistics
    usageData.forEach(usage => {
      // Determine which group this usage belongs to
      // This is an estimation since we don't store the actual selection method
      // We'll consider newer usage (last month) with high composite scores as "composite" group
      const template = templates.find(t => t.id === usage.template_id);
      
      if (template) {
        // Determine which group this template likely belongs to
        const isComposite = (
          template.composite_quality_score > 0.6 && 
          new Date(usage.created_at) > new Date(Date.now() - 30 * 24 * 60 * 60 * 1000)
        );
        
        const group = isComposite ? 'composite_score' : 'efficacy_score';
        
        // Add to group stats
        stats[group].count++;
        
        if (usage.feedback_score) {
          stats[group].ratings.push(usage.feedback_score);
        }
        
        // Check if this usage had a follow-up
        if (usage.had_follow_up) {
          stats[group][`follow_up_count`] = (stats[group][`follow_up_count`] || 0) + 1;
        }
      }
    });
    ...
    Object.keys(stats).forEach(key => {
      ...
      group.avg_rating = group.ratings.length > 0 
        ? group.ratings.reduce((sum, r) => sum + r, 0) / group.ratings.length 
        : 0;
      
      group.follow_up_rate = group.count > 0 
        ? (group.follow_up_count || 0) / group.count 
        : 0;
    });
    ...
    res.json({
      success: true,
      stats,
      note: "This is an estimation based on template scores and usage timing"
    });
  } catch (error) {
...
app.listen(PORT, async () => {
  ...
  try {
    ...
    // Fix RLS policies on startup to ensure proper functionality
    console.log('Checking and fixing RLS policies on startup...');
    try {
      // Helper function to execute SQL that tries both function names
      async function executeSql(sql) {
        try {
      ...
      // Fix sessions table RLS
      console.log('Fixing sessions table RLS policy...');
      const sessionsPolicy = `
      ...
      await executeSql(sessionsPolicy);
      
      // Fix interactions table RLS
      console.log('Fixing interactions table RLS policy...');
      const interactionsPolicy = `
        DROP POLICY IF EXISTS "Users can view their own interactions" ON public.interactions;
        DROP POLICY IF EXISTS "Users can insert own interactions" ON public.interactions;
        DROP POLICY IF EXISTS "Restrictive interactions policy" ON public.interactions;
      ...
      await executeSql(interactionsPolicy);
      
      // Fix prompt_template_usage table RLS
      console.log('Fixing prompt_template_usage table RLS policy...');
      const templateUsagePolicy = `
      ...
      await executeSql(templateUsagePolicy);
      
      console.log('Successfully fixed all RLS policies on startup!');
    } catch (rlsError) {
      console.error('Error fixing RLS policies on startup:', rlsError);
      console.log('System may experience permission issues when storing data.');
    }
  } catch (e) {
...
// Add a route to simulate template selection with different weights and signals
app.post('/api/admin/crowd-wisdom/simulate-selection', async (req, res) => {
  try {
    const { adminKey, topic, weights, useCompositeScore, explorationRate } = req.body;
    ...
    if (!topic) {
      return res.status(400).json({
        success: false,
        message: 'Topic is required for simulation'
      });
    }
    
    console.log(`[Crowd Wisdom] Simulating template selection for topic: ${topic}`);
    console.log(`[Crowd Wisdom] Using ${useCompositeScore ? 'composite' : 'efficacy'} score with exploration rate: ${explorationRate || 0.1}`);
    
    // If weights are provided, recalculate scores first with these weights
    if (weights) {
      console.log('[Crowd Wisdom] Using custom weights for simulation:', weights);
      
      // Create a supervisor instance
      const supervisor = new Supervisor();
      await supervisor.updateCompositeScores(weights);
    }
    
    // Get the ResponseClusterManager to run the simulation
    const selector = ResponseClusterManager;
    
    // Get template options for this topic
    const { data: availableTemplates, error: templateError } = await supabase
      .from('prompt_templates')
      .select('id, topic, efficacy_score, composite_quality_score, follow_up_rate, confusion_score, confidence_score, usage_count')
      .eq('topic', topic);
    ...
    // Run the getTemplateForTopic method to simulate selection
    const selectedTemplate = await selector.getTemplateForTopic(
      topic, 
      useCompositeScore !== false, // Default to true if not specified
      explorationRate || 0.1
    );
    ...
    res.json({
      ...
      selection_method: useCompositeScore !== false ? 'composite_score' : 'efficacy_score',
      exploration_rate: explorationRate || 0.1,
      custom_weights: !!weights
    });
  } catch (error) {
    console.error('Error simulating template selection:', error);
    res.status(500).json({ 
      success: false, 
      error: error.message 
    });
...
// Add a route to run the multi-signal evaluation migration
app.post('/api/admin/run-multi-signal-migration', async (req, res) => {
  try {
    const { adminKey } = req.body;
    // Simple security check - in production, use proper authentication
    if (adminKey !== 'multi-signal-admin') {
      return res.status(401).json({ 
        success: false, 
        message: 'Unauthorized - incorrect admin key' 
      });
    ...
    // Read the SQL migration script
    const migrationScript = await fs.readFile(
      path.join(__dirname, 'db', 'multi_signal_migration.sql'), 
      'utf8'
    );
    
    console.log('Running Multi-Signal Evaluation migration script...');
    
    // Execute the script directly on the database
    const { error } = await supabase.rpc('exec_sql', { sql: migrationScript });
    
    if (error) {
      console.error('Error running Multi-Signal Evaluation migration:', error);
      return res.status(500).json({
        success: false,
        message: 'Migration failed',
        error: error.message
    ...
    console.log('Multi-Signal Evaluation migration completed successfully');
    
    // Calculate composite scores for all templates
    const supervisor = new Supervisor();
    const updatedCount = await supervisor.updateCompositeScores();
    
    res.json({
      success: true,
      message: 'Multi-Signal Evaluation migration completed successfully',
      templates_updated: updatedCount
    });
  } catch (error) {
    console.error('Error in Multi-Signal Evaluation migration route:', error);
    res.status(500).json({
      success: false,
      error: error.message
    });
