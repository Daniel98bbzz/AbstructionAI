...
app.post('/api/query', async (req, res) => {
  try {
    ...
    // Add system context about query analysis if available
    const analysisContext = analysis && analysis.needsDeepening ? `
This is an enhanced query based on analysis indicating the original question needed more depth.
Original query: "${originalQuery}"
Analysis indicators: ${analysis.indicators.join(', ')}
Please provide a more detailed and comprehensive response with specific examples and practical applications.
` : '';

    // Update the system context to include analysis information
    const systemContext = {
      role: "system",
...
Never skip any section of the format. Each section must be properly identified with its header.`
    ...
    // Create suggestions response if deepening is needed
    let suggestionsResponse = null;
    if (analysis && analysis.needsDeepening) {
      console.log('Backend: Creating suggestions response for query:', {
        query,
        analysis,
      ...
      suggestionsResponse = {
        ...
        content: {
          ...
          suggestions: [
            {
              id: 'deeper',
              text: 'Would you like to go deeper?',
              description: 'Get a more comprehensive explanation with advanced concepts'
            },
            {
              id: 'examples',
              text: 'Want me to show real-world examples?',
              description: 'See practical applications and use cases'
            },
            {
              id: 'technical',
              text: 'Prefer a more technical breakdown?',
              description: 'Get a detailed technical analysis with specific details'
            },
            {
              id: 'step-by-step',
              text: 'Need a step-by-step explanation?',
      ...
      console.log('Backend: Created suggestions response:', suggestionsResponse);
    } else {
      console.log('Backend: No suggestions needed for query:', {
        query,
        analysis,
        confidence: analysis?.confidence,
        indicators: analysis?.indicators
    ...
    if (suggestionsResponse) {
      console.log('Backend: Adding suggestions interaction to session');
      await sessionManager.addInteraction(sessionData.id, {
        type: 'suggestions',
    ...
    // Return both responses if suggestions exist
    const finalResponse = {
      main: mainResponse,
      suggestions: suggestionsResponse
    };
    
    console.log('Backend: Sending final response:', {
      hasMain: !!finalResponse.main,
